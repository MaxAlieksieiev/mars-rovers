...
function Game({setIsAuthenticated}) {
    ...
    const fetchFile = async () => {
        if (file) {
            ...
            if (file.length > csvRow) {
                ...
                setCommands(instructions.commands);
                realCommands.current = instructions.commands;
                console.log("realCommands: ", realCommands);
            }
        }
    };
    ...
    const run = () => {
        ...
        if (nextCommand) {
            ...
            alert(`The all rovers done`);
            console.log("counterUndefined: ", counterUndefined);
            console.log("maxCounter: ", maxCounter);
            console.log(0);
            clearTimeout(timeout);
        } else if (nextCommand === undefined) {
            alert(`The ${csvRow + 1} rover done`);
    ...
    const buttonFunction = [run, reset, logOut];
    const buttonTitles = ["Run", "Reset", "Logout"];
